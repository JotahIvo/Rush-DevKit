...
4. For the feature(s) in scope: `prd.md`, `spec.md`, `plan.md`, `tasks.md`, `done-contract.md`,
   and its contracts under `specs/<id>/contracts/` and `specs/shared-contracts/`.
   Also the parent `specs/<spec-id>/prd.md` — the feature PRD's Traceability table cites its
   `FR-NNN` ids, and a citation that does not resolve is a blocker, not a formatting slip.
5. `specs/<spec-id>/architecture.md` — the full architecture of the spec this feature belongs to,
...
3. **Run the judgement layer — always, regardless of step 2's outcome** — because a NO-GO from
   ...
   - **Broken traceability**: any requirement in the feature's `prd.md` whose Traceability row is
     empty or cites an `FR-NNN` that does not exist in the spec's PRD — the feature is building
     something nobody asked for, or the spec's PRD is missing a requirement it should state. Both
     are blockers, and the fix belongs to `/rush-prd` or `/rush-spec`, never to you.
   - **Feature PRD vs spec**: any behaviour `spec.md` describes that no requirement in the
     feature's `prd.md` asks for, and any requirement in that PRD that `spec.md` never realises.
   ...
   - **Architecture not reflected**: any decision in `specs/<spec-id>/architecture.md` or its ADRs
