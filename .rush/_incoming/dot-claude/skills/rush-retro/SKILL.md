...
4. **Retire dead checklist items.** For existing `CLAUDE.md` rules and fitness functions relevant to
   ...
   removal for ones that didn't, with the review period as evidence. Retiring a rule that never
   fired is expected, not a compromise — do it before adding, not after. A checklist nobody reads
   to the end enforces nothing, however many rules it has.
...
8. **Validate.** After editing `CLAUDE.md` and/or `constitution.md`, run
   `.rush/scripts/validate-artifacts.sh --all --json` and fix what it reports. These two files are
   read by every agent on every run: if one is growing past what a person will actually read,
   retire another rule (step 4) rather than letting it accumulate.
...
- [ ] `CLAUDE.md` and `constitution.md` pass `validate-artifacts.sh --all --json`
