...
3. `specs/<feature-id>/`: `prd.md` (the product slice this feature owes — what it must let
   someone do, and which of the spec's requirements it covers), `spec.md`, `plan.md`, `tasks.md`
   (status, verify commands, and its own Session Log — there is no separate `progress.md`),
   `done-contract.md`.
...
is actually being implemented. Creation does not set this cursor for a batch of features, and
nothing else moves it — without this step `session-start.sh`, `/rush-brief` and your own next
session all report a different feature than the one being worked on.
