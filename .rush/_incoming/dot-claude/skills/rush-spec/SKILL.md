---
name: rush-spec
description: Write one feature's whole artifact set in a single pass — its contained product PRD, the technical specification, the implementation plan, the task list and the executable done-contract — generating any contract files its interfaces need along the way. Use when starting work on a feature from the backlog produced by /rush-features.
...
`prd.md` (the product slice — who it is for, which of the spec's requirements it delivers),
...
them). All of them in one pass, because they are one decision seen from five angles. When the spec's Interfaces section declares anything this feature provides, this also
...
5. `specs/shared-contracts/` — interfaces owned by other features. You reference them; you never
   redefine them.
6. `specs/<spec-id>/prd.md` — **the spec's complete PRD**, and specifically the `FR-NNN`
   requirements, journeys and quality attributes that fall inside this feature's boundary. This is
   what the feature's own `prd.md` narrows and cites; you never invent a requirement here that the
   spec's PRD does not state — a gap there is a finding to report, not one to fill in silently.
...
4. **Write `prd.md`, then `spec.md`.** Both are this command's output — the product slice and the
   observable behaviour of the same feature, written together so they cannot drift apart.

   **`prd.md`**, from `.rush/templates/feature-prd-template.md`. Deliberately contained: the
   spec's PRD already holds the full product definition, and repeating it here is how two
   documents start disagreeing. It carries what is true of *this slice only* — who it serves,
   what it must let them do, what it explicitly leaves to a sibling feature, and how anyone
   judges it landed. Its **Traceability** table is the part that earns the file: every requirement
   maps to at least one `FR-NNN` in the spec's PRD, and the section naming the parent requirements
   this feature does *not* cover is what stops two features each assuming the other handled one.
   A requirement here with nothing upstream to cite is scope creep or a gap in the spec's PRD —
   report it either way; never quietly add it.

   **`spec.md`**, from `.rush/templates/spec-template.md`. Required content:
   ...
   Length is whatever the feature honestly needs. But if `spec.md` is sprawling because the
   feature keeps growing new behaviour, that is a scoping signal, not a writing problem — say so
   and propose a split rather than compressing it into something unreadable.
   ...
   The split between the two files this step writes: `prd.md` is product (who, why, which
   requirement), `spec.md` is observable behaviour (interfaces, data, edge cases, failures). An
   endpoint name in `prd.md`, or a user's motivation in `spec.md`, means you are in the wrong file.
...
- feature id and path
- the parent requirements (`FR-NNN`) this feature covers, and any it was expected to cover but
  could not — named, not counted
...
- [ ] `prd.md`, `spec.md`, `plan.md`, `tasks.md`, `done-contract.md` all exist
- [ ] Every requirement in the feature's `prd.md` cites at least one `FR-NNN` from the spec's PRD,
      and the parent requirements left to sibling features are named
