...
2. `specs/<feature-id>/progress.md` — the running log of what has happened.
...
6. `.rush/memory/questions.md` — open questions tied to this feature, unanswered or answered.
7. Debt or blockers noted in `progress.md` or elsewhere in `.rush/memory/` — surface whatever exists;
   do not invent a debt entry that isn't recorded anywhere.
...
2. **Gather state**, one call each: `task-status.sh <id> --list --json`, `done-check.sh <id>
   --json`, `progress.md`, recent git log for the feature's paths, `questions.md` entries that
   mention the feature.
...
4. **Write `brief.md`** from `.rush/templates/feature-brief-template.md`, covering at minimum:
   ...
   - **Open questions and debt**: pulled from `questions.md` and any debt notes found; if none
     exist, say so explicitly rather than omitting the section.
