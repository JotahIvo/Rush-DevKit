...
4. Respect artifact budgets. Density over completeness.
...
**1. Discovery** — run `/rush-pitch`'s process: what the product is, for whom, the problem, what
success looks like, and **the MVP cut** (what is deliberately excluded from v1) with an explicit
appetite. Use `rush-researcher` for comparable products and prior art. Output: `pitch.md`.

**2. Stack decision** — run `/rush-architect`'s process at project level: 2–3 viable stacks with
...
become journey tests.

**6. Breakdown and specs** — run `/rush-features` to produce `specs/integration-map.md` with every
...
each feature in topological order, and `/rush-contracts` for those exposing interfaces. Validate
...
**7. Hand over.** Report the ordered queue and start the first feature with `/rush-implement`.
